...
export function transform(date_range: [Date, Date], schedule: string, first_date: Date): string {
...
function decide(date: Date, records: Record[], first_date: Date): Decision {
    if (date.getDay() === 0 || date.getDay() === 6) {
        ...
        const daysSinceFirstDate = Math.floor((date.getTime() - first_date.getTime()) / oneDay)
        const currentWeek = Math.floor(daysSinceFirstDate / 7) + 1

        const todayRecords = records.filter(record => record.weeks.includes(currentWeek) && record.weekday === date.getDay() - 1)

        const hasMorningClass = todayRecords.some(record => record.start <= 5)
        const hasAfternoonClass = todayRecords.some(record => record.start <= 10 && record.end >= 6)
